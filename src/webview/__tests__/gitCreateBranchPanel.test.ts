...
const { createWebviewPanel, handleRawMessage } = vi.hoisted(() => ({
  createWebviewPanel: vi.fn(() => ({
    webview: {
      ...
      html: 'window.__GITVIEW_APP__="gitCreateBranch"',
...
vi.mock("../getWebviewHtml", () => ({
  getWebviewHtml: vi.fn(
    async () => '<html><head></head><body>window.__GITVIEW_APP__="gitCreateBranch"</body></html>',
  ),
}));
...
function open(repoId: string): Promise<void> {
  return openGitCreateBranchPanel(context, gitView, {
    repoId,
    workspaceRoot: "/ws",
  });
}

describe("openGitCreateBranchPanel", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("answers the webview.ready handshake with its surface", async () => {
    ...
    messageListener()({
      ...
      requestId: "req-1",
      type: "webview.ready",
      payload: { surface: "gitCreateBranch" },
    });
    ...
    expect(sent["payload"]).toEqual(
      expect.objectContaining({ surface: "gitCreateBranch" }),
    );
  });

  it("closes the panel when the webview reports the branch was created", async () => {
    await open("close-repo");
    messageListener()({ type: "git.createBranch.close" });
  ...
  it("routes every other message to the panel router", async () => {
    ...
    const message = { type: "git.createBranch.preview", startPoint: "main" };
  ...
  it("escapes the bootstrap payload so it cannot break out of the script tag", async () => {
    await openGitCreateBranchPanel(context, gitView, {
      repoId: "escape-repo",
      workspaceRoot: "/ws",
      startPoint: "</script><img src=x onerror=alert(1)>",
    });
