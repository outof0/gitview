...
const { createWebviewPanel, handleRawMessage } = vi.hoisted(() => ({
  createWebviewPanel: vi.fn(() => ({
    webview: {
      ...
      html: 'window.__GITVIEW_APP__="gitCommit"',
...
vi.mock("../getWebviewHtml", () => ({
  getWebviewHtml: vi.fn(
    async () => '<html><head></head><body>window.__GITVIEW_APP__="gitCommit"</body></html>',
  ),
}));
...
function open(repoId: string): Promise<void> {
  return openGitCommitPanel(context, gitView, { repoId, workspaceRoot: "/ws" });
}

describe("openGitCommitPanel", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("answers the webview.ready handshake with its surface", async () => {
    ...
    expect(sent["payload"]).toEqual(
      expect.objectContaining({ surface: "gitCommit" }),
    );
  });

  it("closes the panel when the webview reports the dialog was dismissed", async () => {
    await open("close-repo");
    messageListener()({ type: "git.commit.close" });
  ...
  it("routes every other message to the panel router", async () => {
    ...
    const message = { type: "repo.status" };
  ...
  it("escapes the bootstrap payload so it cannot break out of the script tag", async () => {
    await open("</script><img src=x onerror=alert(1)>");
