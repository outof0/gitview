...
export function GitFileIcon({
  ...
  if (glyph) {
    return (
      <span
        className={`inline-flex items-center justify-center text-file-glyph font-bold ${glyph.bg} ${glyph.ink} rounded-vscode ${className}`}
  ...
  if (lower.endsWith(".md") || lower.endsWith(".mdx")) {
    return (
      <svg
        className={`${className} ${FILE_TYPE_GLYPH_FILL.markdown}`}
        viewBox="0 0 16 16"
      >
  ...
  if (
    ...
    return (
      <svg
        className={`${className} ${FILE_TYPE_GLYPH_FILL.image}`}
        viewBox="0 0 16 16"
      >
